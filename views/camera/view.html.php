...
class ConfmgtCkViewCamera extends ConfmgtClassView
{
	...
	*/
	protected function displayDefault($tpl = null)
	{
		...
		//Filters
		// Paper ID > Title + Paper ID > Key words
	...
	*/
	protected function displayModal($tpl = null)
	{
		...
		$this->model		= $model	= $this->getModel();
		$this->state		= $state	= $this->get('State');
		$state->set('context', 'camera.modal');
		$this->items		= $items	= $this->get('Items');
		$this->canDo		= $canDo	= ConfmgtHelper::getActions();
		$this->pagination	= $this->get('Pagination');
		$this->filters = $filters = $model->getForm('modal.filters');
		$this->menu = ConfmgtHelper::addSubmenu('camera', 'modal');
		$lists = array();
		$this->lists = &$lists;
...
JLoader::register('ConfmgtViewCamera', JPATH_ADMIN_CONFMGT .DS. 'fork' .DS. 'views' .DS. 'camera' .DS. 'view.html.php');
