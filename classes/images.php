...
@define("CONFMGT_IMAGES_MAX_WIDTH", 1000);
@define("CONFMGT_IMAGES_MAX_HEIGHT", 1000);
@define("CONFMGT_IMAGES_ALOWED_SIZES", '');
@define("CONFMGT_IMAGES_PHYSICAL_THUMB", true);		//Create an hidden thumb file with parameters
@define("CONFMGT_IMAGES_FALLBACK_NAME", ".notfound.png");
@define("CONFMGT_IMAGES_FALLBACK_ROOT", JPATH_ADMIN_CONFMGT .DS. "images");

@include_once(JPATH_ADMIN_CONFMGT .DS. "classes" .DS. "file.php");


class ConfmgtImages  extends ConfmgtClassFile
{
	...
	function searchFile(&$create)
	{
		...
		$fallback = $this->dir .DS. CONFMGT_IMAGES_FALLBACK_NAME;
		...
		$fallback = CONFMGT_IMAGES_FALLBACK_ROOT .DS. CONFMGT_IMAGES_FALLBACK_NAME;
		if (JFile::exists($fallback))
		{
			$create = 'fly';
			return $fallback;
		...
		switch($create)
		{
			case 'thumb':
				...
				else
				{
					...
					if (($size == "0x0") || in_array($size, explode(",", CONFMGT_IMAGES_ALOWED_SIZES)))
					{
						$image = $this->load();
...
*/
    ...
    function writeThumb($image, $name=null)
    {
    	...
    	if (!CONFMGT_IMAGES_PHYSICAL_THUMB)
	...
	{
		$this->width = min($width, CONFMGT_IMAGES_MAX_WIDTH);
	}

    function height($height='') {
        $this->height = min($height, CONFMGT_IMAGES_MAX_HEIGHT);
    }
