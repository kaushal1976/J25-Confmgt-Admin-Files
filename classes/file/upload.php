...
@define("CONFMGT_UPLOAD_RANDOM_CHARS", 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789');
@define("CONFMGT_UPLOAD_DEFAULT_MAX_SIZE", 2097152);
@define("CONFMGT_UPLOAD_CHMOD_FOLDER", 0744);
@define("CONFMGT_UPLOAD_CHMOD_FILE", 0644);


/**
* Uploader Class for Confmgt.
*
* @package	Confmgt
* @subpackage	Class
*/
class ConfmgtCkClassFileUpload extends ConfmgtClassFile
{
	/**
	* Allowed Files types
	*
	...
	*/
	public static function getMaxSize($string = false, $maxSizeCustom = null)
	{
		...
		if (!$maxSize)
			$maxSize = CONFMGT_UPLOAD_DEFAULT_MAX_SIZE;

		if ($maxSizeCustom)
			$maxSize = min($maxSize, $maxSizeCustom);

		if ($string)
			$maxSize = JText::sprintf("CONFMGT_UPLOAD_MAX_B", self::bytesToString($maxSize));
	...
	*/
	public function process()
	{
		...
		@chmod($fileDest, CONFMGT_UPLOAD_CHMOD_FILE);
	...
	*/
	...
	{
		$lenChars = strlen(CONFMGT_UPLOAD_RANDOM_CHARS);
		$random = "";
		...
		{
			$pos = rand(0, $lenChars);
			$random .= substr(CONFMGT_UPLOAD_RANDOM_CHARS, $pos, 1);
		}
	...
	*/
	public function setUploadFolder($uploadFolder)
	{
		...
		@chmod($uploadPath, CONFMGT_UPLOAD_CHMOD_FOLDER);
		if(!is_writable($uploadPath))
		{
			$app->enqueueMessage(JText::sprintf( "CONFMGT_UPLOAD_PLEASE_MAKE_SURE_THE_FOLDER_IS_WRITABLE",$uploadPath), 'notice');
			return false;
		}
	...
	*/
	public function uploadFile($uploadFile, $options = array())
	{
		...
		if(empty($uploadFile['name'])){
			$app->enqueueMessage(JText::_("CONFMGT_UPLOAD_PLEASE_BROWSE_A_FILE"),'notice');
			return false;
		}
		...
		{
			$app->enqueueMessage(JText::sprintf( "CONFMGT_UPLOAD_THIS_FILE_EXTENSION_IS_NOT_ACCEPTED_THE_ACCEPTED_FILES_ARE",
												$file->extension,
		...
		{
			$app->enqueueMessage(JText::sprintf( "CONFMGT_UPLOAD_MIME_TYPE_NOT_VALID_ALLOWED_MIMES_ARE",
												$this->file->mime,
		...
		{
			$app->enqueueMessage(JText::sprintf( "CONFMGT_UPLOAD_TOO_BIG_FILE_BYTES_MAX_ALLOWED_SIZE_BYTES",
		...
		{
			$app->enqueueMessage(JText::_( "CONFMGT_UPLOAD_THE_FILE_CONTAINS_ERRORS"), 'error');
			return false;
		}
		...
		{
			$app->enqueueMessage(JText::sprintf( "CONFMGT_UPLOAD_THIS_FILE_ALREADY_EXIST",$file->filename), 'notice');
			return false;
		}
		...
		{
			if ($app->isSite())
				$msg = JText::sprintf( "CONFMGT_UPLOAD_COULD_NOT_UPLOAD_THE_FILE", $file->filename);	// Don't show the complete directory in front-end
			else if ($app->isAdmin())
				$msg = JText::sprintf( "CONFMGT_UPLOAD_COULD_NOT_UPLOAD_THE_FILE_TO",$file->tmp,$this->uploadFolder . $file->filename);
...
JLoader::register('ConfmgtClassFileUpload', JPATH_ADMIN_CONFMGT .DS. 'fork' .DS. 'classes' .DS. 'file' .DS. 'upload.php');
JLoader::load('ConfmgtClassFileUpload');
// Fallback if no fork has been found
if (!class_exists('ConfmgtClassFileUpload')){ class ConfmgtClassFileUpload extends ConfmgtCkClassFileUpload{} }
