...
* @subpackage	Classes
*/
class ConfmgtCkModelRevspapersitem extends ConfmgtClassModelItem
{
	/**
	* View list alias
	*
	...
	*/
	public function getLayout()
	{
		...
		return $jinput->get('layout', 'reviewersforpapersitem', 'STRING');
	}
	...
	* @since	1.6
	*/
	public function getTable($type = 'revspapersitem', $prefix = 'ConfmgtTable', $config = array())
	{
	...
	*/
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_confmgt.edit.revspapersitem.data', array());
		...
		if (empty($data)) {
			...
			if ($this->getState('revspapersitem.id') == 0)
			{
				$jinput = JFactory::getApplication()->input;

				$data->id = 0;
				$data->paper = $jinput->get('filter_paper', $this->getState('filter.paper'), 'INT');
	...
	* @param	integer	$pk	The primary id key of the revspapersitem
	* @return	void
	*/
	protected function prepareQuery(&$query, $pk)
	{
		...
		{
			case 'revspapersitem.reviewersforpapersitem':
...
JLoader::register('ConfmgtModelRevspapersitem', JPATH_ADMIN_CONFMGT .DS. 'fork' .DS. 'models' .DS. 'revspapersitem.php');
JLoader::load('ConfmgtModelRevspapersitem');
// Fallback if no fork has been found
if (!class_exists('ConfmgtModelRevspapersitem')){ class ConfmgtModelRevspapersitem extends ConfmgtCkModelRevspapersitem{} }
