...
* @subpackage	Classes
*/
class ConfmgtCkModelCamerareadyitem extends ConfmgtClassModelItem
{
	/**
	...
	* @var array
	*/
	protected $fileFields = array('camera_ready_paper');
	...
	*/
	public function getLayout()
	{
		...
		return $jinput->get('layout', 'camerareadyitem', 'STRING');
	}
	...
	* @since	1.6
	*/
	public function getTable($type = 'camerareadyitem', $prefix = 'ConfmgtTable', $config = array())
	{
	...
	*/
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_confmgt.edit.camerareadyitem.data', array());
		...
		if (empty($data)) {
			...
			if ($this->getState('camerareadyitem.id') == 0)
			{
				$jinput = JFactory::getApplication()->input;
				...
				$data->title = null;
				$data->abstract = null;
				$data->camera_ready_paper = null;
				$data->keywords = null;
				$data->copyright_transfer = null;
	...
	* @param	integer	$pk	The primary id key of the camerareadyitem
	* @return	void
	*/
	protected function prepareQuery(&$query, $pk)
	{
		...
		//FROM : Main table
		$query->from('#__confmgt_camera AS a');
		...
		{
			case 'camerareadyitem.camerareadyitem':

				//BASE FIELDS
				$this->addSelect(	'a.abstract,'
								.	'a.camera_ready_paper,'
								.	'a.copyright_transfer,'
								.	'a.paper_id,'
								.	'a.title');
			...
			case 'camerareadyitem.camerareadyitemview':
...
JLoader::register('ConfmgtModelCamerareadyitem', JPATH_ADMIN_CONFMGT .DS. 'fork' .DS. 'models' .DS. 'camerareadyitem.php');
JLoader::load('ConfmgtModelCamerareadyitem');
// Fallback if no fork has been found
if (!class_exists('ConfmgtModelCamerareadyitem')){ class ConfmgtModelCamerareadyitem extends ConfmgtCkModelCamerareadyitem{} }
