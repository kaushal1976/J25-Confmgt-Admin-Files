...
* @subpackage	Classes
*/
class ConfmgtCkModelTheme extends ConfmgtClassModelItem
{
	/**
	* View list alias
	*
	...
	*/
	public function getLayout()
	{
		...
		return $jinput->get('layout', 'theme', 'STRING');
	}
	...
	protected function loadFormData()
	{
		...
		if (empty($data)) {
			...
			if ($this->getState('theme.id') == 0)
			{
				...
				$data->id = 0;
				$data->track = $jinput->get('filter_track', $this->getState('filter.track'), 'INT');
				$data->name = null;
				$data->description = null;
				$data->theme_page = null;
				$data->ordering = null;
				$data->published = null;
	...
	* @param	integer	$pk	The primary id key of the theme
	* @return	void
	*/
...
JLoader::register('ConfmgtModelTheme', JPATH_ADMIN_CONFMGT .DS. 'fork' .DS. 'models' .DS. 'theme.php');
JLoader::load('ConfmgtModelTheme');
// Fallback if no fork has been found
if (!class_exists('ConfmgtModelTheme')){ class ConfmgtModelTheme extends ConfmgtCkModelTheme{} }
