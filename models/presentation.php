	...
	public function __construct($config = array())
	{
		...
		if (empty($config['filter_fields'])) {
			$config['filter_fields'] = array(
				...
				'publish', 'a.publish',
				'_presenter_paid_and_registered', '_presenter_.paid_and_registered',
	...
	protected function prepareQuery(&$query)
	{
		...
		switch($this->getState('context'))
		{
			case 'presentation.default':
				...
				//BASE FIELDS
				$this->addSelect(	'a.modification_date,'
								.	'a.paper_id,'
								.	'a.presentation,'
								.	'a.presenter');
		...
		//WHERE - SEARCH : search_search_1 : search on Paper ID > Title + Paper ID > Key words + Presenter > First Name + Presenter > Surname + Presenter > Affiliation + Presenter > Email
		$search_search_1 = $this->getState('search.search_1');
		$this->addSearch('search_1', '_paper_id_.title', 'like');
		$this->addSearch('search_1', '_paper_id_.key_words', 'like');
		$this->addSearch('search_1', '_presenter_.first_name', 'like');
		$this->addSearch('search_1', '_presenter_.surname', 'like');
		$this->addSearch('search_1', '_presenter_.affiliation', 'like');
		$this->addSearch('search_1', '_presenter_.email', 'like');
		if (($search_search_1 != '') && ($search_search_1_val = $this->buildSearch('search_1', $search_search_1)))
			$this->addWhere($search_search_1_val);

		//WHERE - SEARCH : search_search_2 : search on Paper ID > Title + Paper ID > Key words + Presenter > First Name + Presenter > Surname + Presenter > Affiliation + Presenter > Email
		$search_search_2 = $this->getState('search.search_2');
		$this->addSearch('search_2', '_paper_id_.title', 'like');
		$this->addSearch('search_2', '_paper_id_.key_words', 'like');
		$this->addSearch('search_2', '_presenter_.first_name', 'like');
		$this->addSearch('search_2', '_presenter_.surname', 'like');
		$this->addSearch('search_2', '_presenter_.affiliation', 'like');
		$this->addSearch('search_2', '_presenter_.email', 'like');
...
JLoader::register('ConfmgtModelPresentation', JPATH_ADMIN_CONFMGT .DS. 'fork' .DS. 'models' .DS. 'presentation.php');
